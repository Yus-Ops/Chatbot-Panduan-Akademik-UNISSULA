...
export default function Chat() {
  ...
  return (
    <div className="flex h-screen flex-col">
      {/* Header */}
      <header className="sticky top-0 z-10 border-b border-slate-200/70 bg-white/80 backdrop-blur dark:border-slate-800 dark:bg-slate-900/80">
        <div className="mx-auto flex max-w-3xl items-center gap-3 px-4 py-3">
          ...
          <button
            ...
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200"
          >
      ...
      {/* Input */}
      <footer className="border-t border-slate-200/70 bg-white/80 backdrop-blur dark:border-slate-800 dark:bg-slate-900/80">
      ...
      {showSettings && (
        ...
        >
          ...
          >
            <h3 className="text-base font-semibold">URL Server (Backend)</h3>
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
              Tempel URL backend di sini (mis. URL Cloudflare Tunnel). Disimpan di
              browser ini — tidak perlu redeploy saat URL berubah.
            </p>
            <input
              value={draftUrl}
              ...
              autoFocus
              spellCheck={false}
              className="mt-3 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-brand-500 dark:border-slate-600 dark:bg-slate-900"
            />
            <p className="mt-2 break-all text-[11px] text-slate-400">
              Default (dari build): {getDefaultApiUrl()}
            </p>
            <div className="mt-4 flex items-center justify-between gap-2">
              <button
                onClick={resetSettings}
                className="rounded-lg px-3 py-1.5 text-xs text-slate-500 transition hover:bg-slate-100 dark:hover:bg-slate-700"
              >
                Reset ke default
              </button>
              <div className="flex gap-2">
                <button
                  onClick={() => setShowSettings(false)}
                  className="rounded-lg px-3 py-1.5 text-sm text-slate-600 transition hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-700"
                >
                  Batal
                </button>
                <button
